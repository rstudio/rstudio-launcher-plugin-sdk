...
The provided sample Local Launcher Plugin implements resource utilization by inheriting from `AbstractTimedResourceStream` because there is no way to stream the information needed from the Operating System directly. The [example below](#todo-16-ex) will show how the `AbstractResourceStream` might be inherited when it is possible to stream the necessary information from the Job Scheduling System.
