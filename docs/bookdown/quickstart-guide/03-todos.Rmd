...
This section will describe the questions which need to be answered to correctly define the capabilities of a job scheduling system, and what value(s) of the `Capabilities` structure should be modified in the overridden implementation of `IJobSource::getCapabilities`.

* Does the job scheduling system support or require containers (such as Docker or Singularity containers)? If yes:
    + set `Capabilities::ContainerCaps::SupportsContainers` to `true`. Note that if `Capabilities::ContainerCaps::SupportsContainers` is set to `false`, all other values of `Capabilities::ContainerCaps` will be ignored.
    + decide how the list of available container images should be determined and add each container image name to `Capabilities::ContainerCaps::ContainerImages`.
    + decide how the default container image should be determined, if any, and then set the value of `Capabilities::ContainerCaps::DefaultImage`.
    * decide whether unknown images should be allowed and set `Capabilities::ContainerCaps""AllowUnknownImages` to `true` or `false` as desired.
...
* Does the job scheduling system support job queues? A job queue is a subset of machines within the job scheduling system. When a job is submitted to the job scheduling system, a specific queue may be requested and the job should be run on one or more of the machines in the requested queue. In some job scheduling systems, this may also be known as a "partition". If yes:
    + pull down the list of queues from the job scheduling system, or otherwise determine the list available queues, and add each queue to `Capabilities::Queues`.
    
* Does the job scheduling system support setting resource limits on a job?
    + determine the type of resource limits that are supported by the job scheduling system. The SDK allows the following resource types: Cpu Count, Cpu Time, Memory, and Memory Swap. For each supported type add a `ResourceLimit` object of that type to `Capabilities::ResourceLimits`, optionally setting the maximum and default values of that resource limit type.
...
* Does the job scheduling system support other placement constraints that are important to surface? It is not necessary to surface every placement constraint supported by the job scheduling system; however if any constraints are commonly used or otherwise important to the end user, they may be surfaced through this feature. Examples of such constraints may be other types of resource limits (e.g. disk space, GPU count, GPU time, etc.), or other types of constraints entirely (e.g. disk type, region of a cloud service, etc.). If yes:
    + fr each placement constraint type, add one `PlacementConstraint` object to `Capabilities::PlacementConstraints` for each possible value of that constraint.
...
```{Rcpp, eval=FALSE}
Error QuickStartJobSource::getCapabilities(
  const system::User& in_user,
  api::Capabilities& out_capabilities) const
{
  // TODO #7: Define cluster capabilities.
  return Success();
}
...
```{Rcpp, eval=FALSE}
Error MarsJobSource::getCapabilities(
  const system::User& in_user,
  api::Capabilities& out_capabilities) const
{
  api::ContainerCapabilities& containerCapabilities = out_capabilities.ContainerCaps;
  containerCapabilities.SupportsContainers = true;
   
  int maxGpuCount = 0, maxCpuCount = 0, maxMemoryMB = 0;
  try
  {
    // Get the container images and the queues.
    containerCapabilities.ContainerImages = mars_api::get_images(in_user.getUsername());
    out_capabilities.Queues  = mars_api::get_queues());
  ...
  for (int i = 0; i <= maxCpuCount; ++i)
    out_capabilities.PlacementConstraints.emplace_back("gpu_count", std::toString(i));
  
  // Populate the resource limits.
  out_capabilities.ResourceLimits.emplace_back(
