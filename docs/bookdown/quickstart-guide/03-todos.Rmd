...
Error MarsJobSource::initialize()
{
  ...
  catch (const mars_api::mars_expcetion& e)
  {
...
* The `mars_api::job` structure has a `_last_modified` field which is a string in the format `YYYY-MM-DDTHH:mm:ss.sssss[+-]HH:mm`. 
* The `mars_api::job` structure has a `_status` field which is a string, and the Plugin developer has added a `marsStatusToStatus` method which converts the Mars job status to it's applicable SDK status equivalent.
* The `mars_api::job` structure has a `_reason` field which is a string describing the reason for the current status.
* The `mars_api::job` structure has an `_id` field which is an unsigned integer, that the Plugin developer has mapped to an SDK `Job::Id` field by converting the integer value to a string.
...
Error MarsJobStatusWatcher::pollJobStatus()
{
  ...
  for (const mars_api::job& job: marsJobs)
  {
    ...
    if (error)
    {
      logging::logErrorMessage("Failed to update job " + std::to_string(job._id), ERROR_LOCATION);
      logging::logError(error, ERROR_LOCATION);
    }
  }
